...
public class DubboRunTimeException extends AbstractValidationException {

	public DubboRunTimeException(JumpStrategy returnHandler) {
		super(returnHandler);
	}
	...
	public void validation(HttpServletRequest request,
		...
		if ("org.springframework.orm.ObjectOptimisticLockingFailureException".equals(exception.getClass().getName()) && "org.hibernate.StaleObjectStateException".equals(exception.getCause().getClass().getName())) {
			if (actionMode) {
				ReturnMessage message = new ReturnMessage(ReturnMessage.FAIL,"10002","您操作的对象，正有用户在操作，请您等待别人操作结束", null);
